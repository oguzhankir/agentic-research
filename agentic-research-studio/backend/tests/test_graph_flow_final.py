...
@pytest.mark.asyncio
async def test_linear_graph_flow_final():
    ...
    async def mock_review_run(state: dict):
        return {"quality_report": "Pass"}
        
    async def mock_html_run(state: dict):
        return {"html_output": "<html>Report</html>"}
    ...
    with patch("app.core.graph.ResearchPlannerAgent") as MockPlanner, \
         ...
         patch("app.core.graph.QualityReviewerAgent") as MockReview, \
         patch("app.core.graph.HTMLDesignerAgent") as MockHTML:
        ...
        MockReview.return_value.run_agent = mock_review_run
        MockHTML.return_value.run_agent = mock_html_run
        ...
        assert result.get("quality_report") == "Pass"
        assert result.get("html_output") == "<html>Report</html>"
